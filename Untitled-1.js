
  {
    name: "SKVET PetShop",
    category: "web",
    description:
      "Sistem manajemen klinik hewan dan pet shop terintegrasi. Memungkinkan pengelolaan pasien hewan, jadwal pemeriksaan, stok produk, dan transaksi penjualan dalam satu platform.",
    technologies: ["CodeIgniter", "Bootstrap", "MySQL", "JavaScript"],
    features: [
      "Dashboard analitik real-time",
      "Manajemen pasien & rekam medis",
      "Sistem jadwal pemeriksaan",
      "Role-based authorization",
    ],
    year: "2023",
    demoUrl: null,
    images: [
      {
        thumb: "1vvy4zJTq_HJ861ZkXVce8_Ei9-rXxY88",
        src: "1vvy4zJTq_HJ861ZkXVce8_Ei9-rXxY88",
        caption: "SKVET PetShop - Front",
      },
      {
        thumb: "13EWYAgXIPMWsxEMU4zI-sCuWzjGKQYlG",
        src: "13EWYAgXIPMWsxEMU4zI-sCuWzjGKQYlG",
        caption: "SKVET PetShop - Admin Dashboard",
      },
      {
        thumb: "1B6feUyF3_vj9Cwcyqd6nWBd-Tp-belWU",
        src: "1B6feUyF3_vj9Cwcyqd6nWBd-Tp-belWU",
        caption: "SKVET PetShop - Periksa Menu",
      },
      {
        thumb: "1MEXBsAMlGRKchIs0f8qePy2qCNrhbJ9p",
        src: "1MEXBsAMlGRKchIs0f8qePy2qCNrhbJ9p",
        caption: "SKVET PetShop - Authorization Management",
      },
    ],
  },
  {
    name: "Simantra Ternak",
    category: "web",
    description:
      "Aplikasi manajemen peternakan digital yang membantu peternak mencatat dan memantau data ternak, kesehatan hewan, serta menghasilkan laporan komprehensif untuk pengambilan keputusan.",
    technologies: ["Laravel", "Vue.js", "MySQL", "Tailwind CSS"],
    features: [
      "Pencatatan data ternak digital",
      "Monitoring kesehatan hewan",
      "Laporan statistik & analitik",
      "Multi-level user access",
    ],
    year: "2022",
    demoUrl: null,
    images: [
      {
        thumb: "1R4qFSPf6A3jzWIX6zdn6o5MRsQso_t5z",
        src: "1R4qFSPf6A3jzWIX6zdn6o5MRsQso_t5z",
        caption: "Simantra Ternak - Front",
      },
      {
        thumb: "1bXRuEN7jJy39aBPCkYnaRHX-JgujdmIY",
        src: "1bXRuEN7jJy39aBPCkYnaRHX-JgujdmIY",
        caption: "Simantra Ternak - Admin Dashboard",
      },
      {
        thumb: "1NnLIomdnADhxyFdSru_JiU_CXPBehB_I",
        src: "1NnLIomdnADhxyFdSru_JiU_CXPBehB_I",
        caption: "Simantra Ternak - Data Ternak",
      },
      {
        thumb: "1E30wACShFTFm8SiB_5bOBHD7W4KP7HUZ",
        src: "1E30wACShFTFm8SiB_5bOBHD7W4KP7HUZ",
        caption: "Simantra Ternak - Authorization Management",
      },
    ],
  },
  {
    name: "Landing Page",
    category: "landing",
    description:
      "Landing page profesional untuk bisnis dengan desain modern dan clean. Menampilkan informasi layanan, portofolio, dan form kontak yang interaktif.",
    technologies: ["HTML5", "CSS3", "JavaScript", "Bootstrap"],
    features: [
      "Desain responsif mobile-first",
      "Animasi scroll halus",
      "Section layanan & portofolio",
      "Form kontak interaktif",
    ],
    year: "2022",
    demoUrl: null,
    images: [
      {
        thumb: "1PUjuHArF3HPYTRWFbbNZwGfOX_UApNNn",
        src: "1PUjuHArF3HPYTRWFbbNZwGfOX_UApNNn",
        caption: "Landing Page - Home",
      },
      {
        thumb: "1sRdcWqT_RVnPvYocbrDw8O75g8abLIyT",
        src: "1sRdcWqT_RVnPvYocbrDw8O75g8abLIyT",
        caption: "Landing Page - Services",
      },
      {
        thumb: "1I3hLV9JQy8JW5Q8KI_CfPxq8FMsA1pYd",
        src: "1I3hLV9JQy8JW5Q8KI_CfPxq8FMsA1pYd",
        caption: "Landing Page - Footer",
      },
    ],
  },
  {
    name: "HRL - ERP",
    category: "erp",
    description:
      "Sistem Enterprise Resource Planning (ERP) komprehensif untuk PT HRL. Mengintegrasikan berbagai modul bisnis termasuk HR, inventory, keuangan, dan pelaporan dalam satu platform terpadu.",
    technologies: ["CodeIgniter", "Bootstrap", "MySQL", "jQuery", "Chart.js"],
    features: [
      "Dashboard eksekutif",
      "Modul HR & penggajian",
      "Manajemen inventory",
      "Laporan keuangan terintegrasi",
    ],
    year: "2023",
    demoUrl: null,
    images: [
      {
        thumb: "1pQX9C6R-fTVifTe3bor7Di9ysiNRMjq-",
        src: "1pQX9C6R-fTVifTe3bor7Di9ysiNRMjq-",
        caption: "HRL - ERP - Dashboard",
      },
    ],
  },
  {
    name: "HRL Pelatihan",
    category: "web",
    description:
      "Sistem manajemen pelatihan karyawan untuk PT HRL. Memungkinkan HR mengelola jadwal pelatihan, peserta, materi, dan sertifikasi dengan efisien.",
    technologies: ["CodeIgniter", "Bootstrap", "MySQL", "JavaScript"],
    features: [
      "Manajemen jadwal pelatihan",
      "Registrasi peserta online",
      "Tracking kehadiran",
      "Penerbitan sertifikat digital",
    ],
    year: "2023",
    demoUrl: null,
    images: [
      {
        thumb: "1Hf1niXsGT4lI_ImVDtORuGFF0xjqYibw",
        src: "1Hf1niXsGT4lI_ImVDtORuGFF0xjqYibw",
        caption: "HRL Pelatihan - Dashboard",
      },
    ],
  },
  {
    name: "HRL Perpustakaan",
    category: "web",
    description:
      "Sistem perpustakaan digital untuk PT HRL. Memudahkan karyawan mencari, meminjam, dan mengembalikan buku serta dokumen perusahaan secara online.",
    technologies: ["CodeIgniter", "Bootstrap", "MySQL", "JavaScript"],
    features: [
      "Katalog buku digital",
      "Sistem peminjaman online",
      "Notifikasi jatuh tempo",
      "Dashboard statistik peminjaman",
    ],
    year: "2023",
    demoUrl: null,
    images: [
      {
        thumb: "1qR1hIY0ch7ABwIYvQ9y5xlRbZZZ5dQrK",
        src: "1qR1hIY0ch7ABwIYvQ9y5xlRbZZZ5dQrK",
        caption: "HRL Perpustakaan - Dashboard",
      },
    ],
  },